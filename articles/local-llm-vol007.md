...
最初はラズパイ5を考えていた。ラズパイも色々揃えると結構な値段になるので、それなら24回分割、金利0で購入できるという誘惑に負けてMac miniが良いとなりとうとう買ってしまった。

一番低グレードの16GB統合メモリ。Apple Siliconでollama run gemma4:e4bが軽快に動く。コンパクトで静かで消費電力も少ない。
...
だから**Pythonスクリプトに切り替えた。** Discord.pyでgrokの投稿を監視して、Ollamaに投げて、結果を返す。最小限の構成だ。
...
何か24時間作業してくれる自分の分身みたいな存在ができて感動している。ここからパイプラインをどう繋いでいくか考えると脳汁が出る。
